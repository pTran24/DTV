...
<html>
	...
	<body>
		<div id="banner">
            ...
            <div id="bannerText">SOATest License ALPHA</div>
        </div>
		<?php
			$csv_output = "";
			...
			#$sqlSelect = "SELECT DATE_FORMAT(`time`, '%Y-%m-%d') as DATE, concat(`username`,' - ',`email`) as INFO,`hostname` as HOST, count(*) as REFRESHES FROM `licenselog` l, `userinfo` u WHERE l.username = u.userid AND `time` between '2014-06-25' AND '2014-07-02' group by DATE_FORMAT(`time`, '%Y-%m-%d'), username order by DATE_FORMAT(time, '%Y-%m-%d') asc, REFRESHES desc";
			$sqlSelect = "SELECT DATE_FORMAT(`time`, '%Y-%m-%d') as DATE, concat(`username`,' - ',`email`) as INFO,`hostname` as HOST, count(*) as REFRESHES FROM `licenselog` l, `userinfo` u WHERE l.username = u.userid AND`time` > NOW() - INTERVAL 1 WEEK group by DATE_FORMAT(`time`, '%Y-%m-%d'), username order by DATE_FORMAT(time, '%Y-%m-%d') asc, REFRESHES desc";
			...
			while ($row = mysqli_fetch_assoc($noTokenResult)){
				#array_push($noTokenArr, join(" ", $row));
		...
		<!-- Legend Table-->
			...
			<div class='legendItem redbg' >Unusually Long Usage</div>
