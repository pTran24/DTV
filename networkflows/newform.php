...
EDITVALUES;
			...
			<?php //PROCESS INPUT
				/*
				$canSubmit = (notEmpty($environmentPost) && notEmpty($typePost) && notEmpty($sourceLocationPost) && notEmpty($sourcePost)
							&& notEmpty($sourceIPPost) && notEmpty($destinationPost) && notEmpty($destinationLocationPost) && notEmpty($destinationIPPost)
							&& notEmpty($destinationPortPost) && notEmpty($protocolPost) && notEmpty($passwordPost));
				$allEmpty = (!notEmpty($environmentPost) && !notEmpty($typePost) && !notEmpty($sourceLocationPost) && !notEmpty($sourcePost)
							&& !notEmpty($sourceIPPost) && !notEmpty($destinationPost) && !notEmpty($destinationLocationPost) && !notEmpty($destinationIPPost)
							&& !notEmpty($destinationPortPost) && !notEmpty($protocolPost));			
				if (!($allEmpty) && !($canSubmit)){
					echo "<script>alert('You must fill in all required fields to proceed')</script>";
				}
				else if (notEmpty($passwordPost) && $canSubmit && $passwordPost != $masterPassword){
